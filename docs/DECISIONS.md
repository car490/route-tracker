...
| BusOps Announce product tiers | **Three tiers, decided 2026-09-01**: `BusOps Announce` (Driver → Controller → Announce, the full Controller-based architecture), `BusOps Announce Lite` (Driver → Announce, no Controller, paired over Supabase Realtime), `BusOps Announce Solo` (Announce only, no Driver device, geofence+time schedule-autopilot). Lite and Solo are the same tablet-based hardware/software, distinguished only by whether a Driver device is linked (reversible at any time) — not separate builds. Supersedes the doc's earlier "Standard"/"Lite" (with "Lite" covering both paired and driverless installs) naming. Code identifiers realigned the same day: `announceStandaloneAutopilot.js` → `announceSoloAutopilot.js` (`startSoloAutopilot`), `announceLiteFeed.js` → `announceDeviceFeed.js` (`connectAnnounceDeviceFeed`, shared by both tiers), `announceLiteSetup.js` → `announceDeviceSetup.js`; comments/UI text (`AnnounceDeviceLinkPage.jsx`, `docs/TESTING.md` §17, `docs/HARDWARE.md` §14, SQL comments) updated to match. | `docs/ANNOUNCE-PRODUCT-TIERS.md` |
...
| Shared journey-tracking core (`shared/gps.js`/`geofence.js`/`engine.js`) across Driver, Announce Lite, and Announce Solo's own schedule-autopilot layer | **Still open, flagged not decided, 2026-09-01.** As more tiers/variants of "run the tracking loop" appear, there's a real future question of whether this becomes its own shared package/service instead of being imported per-surface as it is today. Not worth doing with only three consumers — revisit once a third or fourth genuinely distinct consumer makes the shared-vs-duplicated tradeoff concrete. | `docs/ANNOUNCE-PRODUCT-TIERS.md` "Beta-readiness pass, 2026-09-01" |
| Announcement audio generation + distribution | **Decided and shipped, 2026-09-08 design / 2026-09-15–16 implementation, fully live on dev AND production as of the `v2.2.0` release.** Replaced the manual "developer runs a script, commits clips to the repo" process with a fully automatic server-side pipeline for the Driver PWA and Announce Solo (both browser-based): a DB trigger on `stops`/`routes` enqueues a job the moment relevant text changes, a capped Supabase cron drains it through an Edge Function (Azure Neural TTS) into a public Storage bucket + `announcement_clips` table, and both tiers read from that live table/bucket through the same shared `shared/announcementAudio.js` code. Separately, the live `speechSynthesis` fallback was **eliminated outright** on those two tiers (not just made rarer) — a stop with no confirmed clip now plays no audio and raises a queryable `announcement_coverage_gap` ops alert instead, with a non-blocking driver-facing warning at journey start. **The Bus Controller (a third, separate audio-playback path) deliberately stays on the old local-generator-plus-committed-files model** — it has no WAN path, so it can never read the live pipeline; `scripts/generate-announcement-audio.mjs` and `busops/driver/audio/announcements/` remain permanent infrastructure for that one consumer, not a transition artifact. A real bug in that Controller path (`mele-server/audioPlayer.mjs` resolving its clip directory one level too shallow, silently skipping every announcement since commissioning) was found and fixed the same day. | `docs/ANNOUNCEMENT-AUDIO-SYNC-PLAN.md` (full rollout ledger), `CLAUDE.md` "PSVAIR announcement audio" |
| Announce sign text sizing (PSV(AI)R Reg 14(4), 22 mm) | **Decided 2026-09-19 (owner's reading), shipped in PRs #65–#68.** Strict reading: **22 mm lowercase x-height** on Lines 2 and 3 (the locale and the actual stop); everything else on the sign is smaller and fixed. That reading is the owner's and is **not yet confirmed against DfT guidance**. Sized physically: a panel profile that carries its measured lit height (Solo tablet 289 × 180 mm, `litHeightMm: 180`) has Lines 2/3 computed from that height and the loaded font's measured x-height, so they land on **22.1 mm** (the 22 mm rule plus a 0.1 mm margin: sized to exactly 22.0 mm by canvas, the real tablet's shortest drawn letter measured 21.92 mm on 2026-09-19). A profile with no measured height keeps the old diagonal-based `computeMinTextVh()`, which sizes the *font* (not the x-height) to 22 mm — about 11 mm of x-height, **not compliant under this reading** until its lit height is measured and added. | `announce/src/panelSizing.js`, `mele-server/DEPLOY.md` "Panel physical size", `docs/HARDWARE.md` §6 |
| Announce sign — every other text size | **Decided 2026-09-19.** Top bar text and Line 1 share one size, 13.5 mm; the top bar is 23 mm deep; Line 1 sits in a fixed 21.6 mm slot that also holds the "wait here" box (title 8.1 mm, message 5.7 mm); terminus, diversion and the no-comma sentence are 24 mm; the brand mark's main line is 5.5 mm (its sub-lines keep their ratios). All defined in mm and converted to vh from the lit height. Route start is three lines like every other stop ("This is an S116T to" / locale / stop); a destination with no comma keeps the sentence. Spoken text is unchanged. | `announce/onboard.css`, `announce/src/headlineLines.js` |
| Scrolling long stop names | **Allowed, not preferred** (owner, 2026-09-19: DfT would rather text did not scroll, but scrolling is not prohibited). The sign keeps the 22 mm size and scrolls a line that is too wide rather than shrinking it. So where a shorter honest stop name exists, shorten it (data fix on its own branch, driven by the stop-name review tooling) instead of accepting a scroll. | `scripts/announce-replica/stop-review/` |
| Panel lit heights | **Solo tablet (LEVIRTU 14"): 289 × 180 mm, measured.** **Bar (28"): unknown.** The Dell-based `monitor` profile is **not in use**, and stays on the legacy sizing. Until a panel's lit height is measured and put in its profile it keeps the legacy sizing. The replica harness's true-size claim still rests on laptop ruler readings not yet taken. | `announce/src/panelSizing.js` `PANEL_PROFILES` |
...
| BETA passenger display | **The LEVIRTU 14" Android tablet is the display in use** (the Announce Lite/Solo tablet — see its row below). **The Dell Pro P2426H is not being used** (owner, 2026-09-19); it was bought 2026-08-14 (no stand) for a mains-powered demo build and stays in the record as history only. What a Controller-fed HDMI panel would use is not named anywhere in this ledger now. | `docs/HARDWARE.md` §6 |
...
| Announce Lite/Solo tablet (passenger-display + tracking device, shared by both tiers) | **DOOGEE Tab E3 Max proposal superseded, 2026-09-01 — never purchased.** Actual beta hardware: **LEVIRTU 14" Android tablet** (OEM: PIXGOOD M328-EEA, Android 16/SDK 36, 4G LTE/SIM + 5G WiFi), already purchased by the user and provisioned/live-tested this session — see `SOLO-DEVICE-SETUP.md`. **Still in tension with the mount decision below** — a 14" tablet doesn't fit the 9"–11" stanchion mount class either, carried forward unresolved, not solved by buying different hardware. | `SOLO-DEVICE-SETUP.md`, `docs/HARDWARE.md` §14 |
| Announce Lite/Solo tablet — mount | **Decided: stanchion clamp, metal preferred**, not a ceiling/ceiling-void mount. Two proposed candidates (RAM X-Grip + Tough-Claw 9" Pipe Mount; iBOLT TabDock LockPro 38mm AMPS), both capped at 9"–11" tablets — conflicts with the 14" tablet actually purchased (row above), unresolved. | `docs/HARDWARE.md` §14 "Mount" |
| Announce Lite/Solo — seat-visibility compliance (§1.1–§1.3) | **Still open, flagged not invented.** ≥22mm text (§1.4) is sized physically from a panel's measured lit height (see "Announce sign text sizing" above); a smaller screen spends more of its own height on mandatory-size text but does not fail §1.4. ≥51% seat visibility does not have a size formula anywhere in this repo — the original proposal assumed central-aisle ceiling mounting to satisfy it, and never confirmed a percentage for any candidate. A stanchion-height mount is a different geometry than that assumption, not fixed by picking a bigger screen. Needs an actual in-vehicle sightline check before a stanchion install is confirmed for anything beyond a small vehicle. | `docs/HARDWARE.md` §14 "Compliance risk" |
