...
| Announce Lite/Solo — seat-visibility compliance (§1.1–§1.3) | **Still open, flagged not invented.** ≥22mm text (§1.4) scales fine to any screen size via `computeMinTextVh()`. ≥51% seat visibility does not have a size formula anywhere in this repo — the original proposal assumed central-aisle ceiling mounting to satisfy it, and never confirmed a percentage for any candidate. A stanchion-height mount is a different geometry than that assumption, not fixed by picking a bigger screen. Needs an actual in-vehicle sightline check before a stanchion install is confirmed for anything beyond a small vehicle. | `docs/HARDWARE.md` §14 "Compliance risk" |
