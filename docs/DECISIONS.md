...
| Announcement audio generation + distribution | **Decided and mostly shipped, 2026-09-08 design / 2026-09-15–16 implementation.** Replaced the manual "developer runs a script, commits clips to the repo" process with a fully automatic server-side pipeline: a DB trigger on `stops`/`routes` enqueues a job the moment relevant text changes, a capped Supabase cron drains it through an Edge Function (Azure Neural TTS) into a public Storage bucket + `announcement_clips` table, and both Driver and Announce Solo read from that live table/bucket through the same shared `shared/announcementAudio.js` code (closing the "two independently-implemented audio paths" risk, not just the generation gap). Separately, the live `speechSynthesis` fallback was **eliminated outright** (not just made rarer) — a stop with no confirmed clip now plays no audio and raises a queryable `announcement_coverage_gap` ops alert instead, with a non-blocking driver-facing warning at journey start. Rollout is genuinely two-speed: generation is live on both dev and production; the never-synthesize behavior and reading from the live pipeline are dev-only so far. | `docs/ANNOUNCEMENT-AUDIO-SYNC-PLAN.md` (full rollout ledger), `CLAUDE.md` "PSVAIR announcement audio" |
