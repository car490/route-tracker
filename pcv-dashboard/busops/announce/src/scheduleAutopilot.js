 ...
 * standalone device (announceStandaloneAutopilot.js). testing_mode,
 * terminus_radius_m, and the match-window fields all take effect simply by
 * the caller replacing its deviceRow reference — tryMatch() reads them
 * fresh every idle-poll tick, no extra signal needed. candidate_departure_ids
 * is the one field that needs an explicit flag, since honoring it requires
 * an actual network re-fetch (schedule_view lookup via refreshCandidates()).
 * gps_source is flagged separately so a device linked/unlinked mid-session
 * (the "hot-switch" announceLiteFeed.js's own header previously disclaimed)
 * can tear down one mode and start the other.
 *
